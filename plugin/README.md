...
Override the path to the helper with `PLAN_REVIEW_SCRIPT=<absolute>` if you're not running from a clone.
...
│   ├── tui-plugin.ts             # TUI-side plugin (Tab observer + model snapshot)
