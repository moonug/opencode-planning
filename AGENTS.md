...
- Picker capture: `context.event.subscribe({signal})` on setup, filter `session.model.selected`. The envelope is `{type, data: {sessionID, model}}` — `data`, not V1-style properties, and NO agent field. Attribute the pick to the session's current agent via `session.get({sessionID}).agent` (projector-maintained); hardcoding `"build"` would write plan-agent picks into the build slot, i.e. the V1 plan-model leak. The subscription reconnects after a 5s delay on stream error and is aborted by the cleanup returned from `setup`.
- Echo guard: `exitPlanMode`'s own `switchModel` is broadcast back as `session.model.selected`. `writePicker` skips an IDENTICAL model (no-op, so a pinned entry keeps its `pinned` flag and `source`) — that equality rule carries the case; the `PickerGuard` window passed into `exitPlanMode` is defense-in-depth and is cleared in `finally` so a throwing switch cannot latch it and swallow the user's real picks.
...
- Commands: opencode2 declares slash commands in the `commands` key of the config document (`{template, description?, agent?, model?, subtask?}`; `$ARGUMENTS`/`$1..$n` are substituted, input is appended when the template has no placeholder) or as markdown files under `<config dir>/{command,commands}/**/*.md`. `CommandDraft` has no `add`, so a plugin cannot register commands — `plugin/commands-v2.jsonc` is the snippet users merge into `~/.config/opencode-v2/opencode.jsonc`, and the v2 tools (`set_build_model`, `plan_diag`) replace the V1 slash commands.

**Known residuals (not blocking, do at the next metadata migration)**
- The host validates metadata keys against `^[a-z0-9][a-z0-9._-]*$`; `planReviewModels` starts lowercase and is fine, but keep the rule in mind when renaming.
